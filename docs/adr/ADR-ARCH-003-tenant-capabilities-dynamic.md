...
Un SaaS real exige que cada tenant porte su set `capabilities` en D1 y lo reciba en `GET /api/auth/session` hacia `capabilitiesStore` reactivo, con 3 niveles aislados: SuperAdmin `admin.kipuspay.com` prod (staging `pages.dev` hasta compra de `kipuspay.com`, ver vars `ALLOWED_PLATFORM_ORIGINS` en `wrangler.jsonc` — sin wildcard `*.pages.dev`), Dueño `app.kipuspay.com/owner` prod / `kipuspay-app.pages.dev/owner` staging, POS `/` (caja offline-first). Sin esto, facturación por plan es incorrecta y cualquier claim GTM (GTM §4.1) es falso.

## Decisión

**SoT = `tenant_capabilities` (D1 `tenant_id, capability, enabled, config_json` PK)** + `tenants.plan_id` como input comercial. `tenants.plan_id` mapea a defaults en `domain-billing` (`plan→capabilities`), overrides por tenant viven como filas. Capabilities viajan **solo** por `GET /api/auth/session` extendido `{capabilities: string[] sorted, capabilitiesEpoch: number}` (vista de D1, cache KV 10s `control-plane.ts:283` PERF-04, nunca JWT), poblando `capabilitiesStore` (Svelte `Set<string>`) con persistencia IDB + `tenant_data_epochs.epoch` (`0035:362-364` triggers). Gating: UI `store.has(cap)` y API `CapabilityResolver` reconsulta `tenant_capabilities` — ambos deben tener `enabled=1` y epoch válido. `FEATURE_*`/`PUBLIC_FEATURE_*` solo son kill-switches: únicamente un `"0"` revoca globalmente; su ausencia nunca concede acceso. SuperAdmin en control plane aislado `admin.kipuspay.com` prod / `pages.dev` staging hasta compra dominio (CF Access, `platform_admin ≠ owner`, `x-tenant-id` jamás sustituye JWT), Dueño solo `PATCH /api/tenant/plan` (derivado), nunca toggle fino.
...
- **Activación:** Ola 0 ADR+Ledger 0525 Aceptado → Ola 1 DDL `0036` + backfill + `FEATURE_TENANT_CAPABILITIES_DYNAMIC=0` → Ola 2 session/store `=1` canario staging → Ola 3 SuperAdmin `admin.kipuspay.com` prod / `pages.dev` staging hasta compra dominio → Ola 4 plan upgrade reconciliación → Ola 5 kill-switch a `1` prod + deprecate `PUBLIC_FEATURE_*` 1 release después. **NOTA IMPORTANTE (DAT-10): DDL 0036 citado en v1 es 0064 en repo (0064_ola1_tenant_capabilities_backfill.sql) — Ledger 0526:15336, INDEX.md:174 §5.3:63. La numeración 0036 era reserva, no migra. Dominio `kipuspay.com` pendiente de compra — staging canónico `pages.dev` (ver `wrangler.jsonc` staging `ALLOWED_PLATFORM_ORIGINS` con hosts `pages.dev` explícitos, sin wildcard), migración a `admin.kipuspay.com` documentada en runbook.**

## Evidencia de cierre

- Tests / checks: `V-07`/`V-23` 0 `switch(vertical)`, `V-05` `tenant_id NOT NULL`, `V-25` up↔down espejo, `V-29` 3 triggers `tenant_capabilities`, `V-28` `/api/auth/session` registrado, `V-24` `320kB` `SIZE_LIMIT` + `bundle_budget.py` (ADR-0040), `V-13` cadena `0525` `b1dd...→???`, `V-08` registry sin huérfanos, `scripts/verify.sh SUITE GREEN` + `scripts/quality.sh` CAL-03 95% dominio (`plan-defaults.test.ts`, `tenant-capabilities.test.ts`), CAL-05, semgrep `tenant_id from jwt only`.
- Ledger: `id: 0525`
