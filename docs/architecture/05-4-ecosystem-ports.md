...
3. **Export contable:** `AccountingExporter` es de solo lectura sobre ventas/CxC/CxP ya ACID; no altera el ledger al exportar. **Alcance (C5):** cubre los documentos de venta `NV`/`01`/`03`/`12` (facturas, boletas y NV); las **NC/ND** (`07`/`08`) quedan **fuera de alcance por diseño** en este sprint — nunca se exportan como ventas. **Desglose por método (C4):** el débito se reparte por el desglose real de pagos (`sale_payments` → `payment_methods.code`): todo lo no-crédito (efectivo, yape, tarjeta, transferencia) suma a la cuenta `1011`; el saldo a crédito y el remanente no pagado (`accounts_receivable`) van a `1212`.
4. **API pública:** autenticación por API key de tenant; webhooks con HMAC; eventos mínimos `sale.created`, `cpe.accepted`, `cpe.rejected`; capability `integrations.api` (Plan Guard **403 `PLAN_REQUIRES_CADENA`** en rutas API — semántico, nunca 402 porque el cobro de la venta jamás es la barrera — y nunca en cobro). Sin `API_KEY_PEPPER` configurado las rutas de API keys responden `503 PEPPER_UNAVAILABLE` (fail-closed, nunca pepper conocido). **Drain/reproceso (C7):** `POST /api/integrations/webhooks/drain` es operación de mantenimiento **solo admin/owner** (403 `FORBIDDEN_ADMIN` en otro rol). **Firma sin secreto (C8):** si el secreto HMAC del endpoint no existe en KV al entregar, la entrega se marca **fallida** — jamás se firma con `''` como si fuese un secreto legítimo.
