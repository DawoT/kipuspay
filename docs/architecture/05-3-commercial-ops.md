...
32. **LPDP y DR/BCP (`compliance.lpdp`, `platform.dr`, FASE 6F):** (a) **LPDP Perú** (reglas LPDP-01..04; ADR-0031; Ley N.º 29733): inventario de PII (nombre, email, teléfono, dirección, RUC/DNI) en `customers`, consentimiento explícito **por propósito** en `consent_records` (migración 0040; reusa y migra el opt-in de mensajería Sprint 24), derecho de export y **borrado/anonimización** — los doc fiscales se retienen (SUNAT ~5 años) pero **se anonimizan** (`[ANONYMIZED]`/`00000000` en el snapshot, `pii_erased` en `customers`), no se destruyen; el motor ACID bloquea la re-materialización (`LPDP_ERASE_BLOCK`); (b) **DR/BCP**: RPO=0 en tx ACID comprometidas, RPO≤1d en rollups, **RTO objetivo ≤30 min por shard**, backups versionados (regla 27) con **restauración aplicada a un shard DR aislado** (binding `DR_DB` por composición, jamás producción viva; `INSERT OR IGNORE` por PK, idempotente) y **simulacro anual** (`POST /api/dr/simulation`, owner + step-up): restore → verificación RPO/RTO → replay de colas sin duplicados → `audit_events` `DR_SIMULATION_*` con `rto_ms`, `rpoTxZero`, `rpoRollupOneDay` y `replayDuplicatesBlocked` (extiende Sprint 14; game day chaos `dr-failover`).
...
`config_json` se interpreta únicamente server-side mediante `CapabilityConfig`.
El resolver rechaza JSON no objeto, versiones inválidas y tipos/rangos
específicos (por ejemplo, precios en cents, tasas en basis points, stock en
microunits y catálogo de combustible). El cliente recibe solo configuraciones
ya validadas; una configuración inválida produce `503` fail-closed y nunca
habilita un módulo por omisión.
...
-- La línea genérica de un despacho de Grifos no omite inventario: el volumen
-- se descuenta de fuel_catalog.stock_microunits en el mismo batch de la venta.

-- FASE 6H / Grifos — catálogo y despacho server-authoritative.
-- El precio e IGV se resuelven desde estas filas; el cliente solo propone volumen.
...
38. **Catálogo vendible en la terminal (`catalog.sellable`, Sprint C1):** el POS muestra en su pantalla de venta un **grid de catálogo vendible** servido por `GET /api/catalog/sellable`: solo `products` con `is_active = 1 AND is_sellable = 1 AND deleted_at IS NULL`, con **precio resuelto por lista** (lista de la sucursal del JWT → lista default del tenant → `price_cents` del catálogo; para variantes: `variant_price_override_cents` → precio de lista del padre → precio de lista propio → precio de catálogo, patrón `resolveVariantUnitPriceCents`), **stock por sucursal** (`branch_product_stock.stock_microunits`, suma agregada si no hay sucursal) y UOM base. El grid es **lectura Zero-Trust**: el precio mostrado es informativo; el precio final lo impone SIEMPRE el servidor al procesar la venta (regla 1). **Offline:** sin red, el grid no bloquea la venta — se muestra el estado sin conexión y queda disponible la **venta rápida genérica** (edge 2A, regla 34); no se cachea catálogo en IDB. La terminal muestra el grid al cajero autenticado (cualquier rol con tenant); la autorización exige `tenant_capabilities` y el flag `FEATURE_CATALOG_SELLABLE` solo funciona como kill switch global.
