...
| **GTM-16** | Catálogo multi-variante / unidades de medida + kits BOM + listas de precio (Crece) | Kits BOM + listas: **listo tras QG Sprint 18**; multi-variante/UM: congelado hasta Sprint 31 | 0 stock cruzado entre variantes; conversión UM exacta (redondeo servidor, sin `toFixed`); explosión BOM atómica con rollback total; precio resuelto por lista |
...
- *"¿Manejan tallas, colores o unidades de medida?"* → El catálogo multi-variante y las unidades de medida se comunican como claim solo después del Quality Gate del **Sprint 31** (GTM-16). Antes, productos simples sin variantes; la conversión de unidades se resuelve en el servidor.
