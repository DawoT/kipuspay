...
**Cierre CLOSED requiere:** 2 impresoras térmicas físicas (1×58 mm + 1×80 mm, dos marcas/modelos distintos documentados) + 2 Android Go reales (1 GB RAM) + fotos/tickets + evidencia headed del pairing UI ya implementado en C7 + pruebas disconnect/timeout + 500 ventas offline con `doze`/`F5` + firmas A+V humanas. Hasta entonces el claim **"etiquetas de precio en impresoras/perfiles compatibles"** permanece congelado y `pos_terminals.paper_width_mm` no se activa en producción (capability `catalog.price_labels` default-off).
...
- `pending-batches.yaml` bloque `go-live-hardware`: `AGENDADO_AL_FINAL`, gates `[s41, GTM-26]`, gaps `hw-printers-matrix` (G2 WAIT por laboratorio físico; pairing UI C7 DONE en software) y `hw-android-offline` (G2 WAIT, 500 ventas gama baja, depende G1 `fcm-vapid-real`).
...
| **G2.4** | Pairing UI E2E (PrinterPairing.svelte) | Flujo de software implementado: ① `scanWebUsb` gesto → chooser → "Impresora USB emparejada." ② `saveWss` con validación `wss:` + host ③ `paperWidthMm` 58/80 persistido en `localStorage kipuspay:pos-terminal-*` + `POST /api/pos/terminals/pairing` ④ `preflight` lista refleja pairing ⑤ `runTestPrint` usa la ladder; contrato cubierto por tests de pairing/transport. | Falta video/screenshot en Chrome/Edge real, chooser USB, respuesta HTTP 200 y `audit_events HARDWARE_DIAG`; esa evidencia pertenece al laboratorio físico y no se simula. | **R: Hardware+FE** | **A: Principal** | **V: QA ind.** | **DONE software / WAIT headed-físico** |
