...
|---|---|---|---|---|---|---|
| Emisión SUNAT en vivo (GTM-07/08) | `fiscal.cpe` | GREEN local (pipeline verificado real, batch I) | submit HTTP→PSE, CDR, drain outbox→R2→SENT, DLQ | Certificación con el entorno SUNAT/OSE real; firma A+V | GTM-08 / FASE 8 | Staff Fiscal + Staff Security |
...
| Export "cuando quieras" + LPDP ARCO (GTM-09) | `compliance.lpdp` | GREEN local (admin sellado batch H) | consent/export/erase, simulacro LPDP | staging R2/Workflow/Secrets/KMS reales; política publicada; A+V | s42/s47 | Staff Security + Staff Data |
...
| Comandas/KDS + salón + split | `orders.kds` | Motor GREEN (domain + worker API) — UI en Sprint C2 | off-banner protegido (frozen-features) | UI de cocina/salón sellada real (C2); luego staging externo | C2 + s43 | Staff QA + Staff Mobile |
