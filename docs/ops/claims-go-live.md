...
|---|---|---|---|---|---|---|
| Analítica predictiva Cadena (GTM-01) | `analytics.forecasting` | GREEN local + canary parcial de gating/refresh | Contrato Holt-Winters, no mutación automática y gating Cadena | MAPE medido contra objetivo aprobado por Staff Data + PM; cron/canary externo, QA humana y firmas A+V. Sin umbral acordado y medido, no hay aceptación | s46 | Staff Data + Staff PM |
...
| Export "cuando quieras" + LPDP ARCO (GTM-09) | `compliance.lpdp` | GREEN local condicionado; claim **CONGELADO** | consentimiento/export/erase admin + `/lpdp` titular; OTP por correo y limitación atómica SQLite DO GREEN local | No ampliar canary ni habilitar autoservicio; falta sender KipusPay real, revisión Security final del diff, validación en staging, QA y firmas A+V. Staging previo es solo evidencia histórica | s42/s47 | Staff Security + Staff Data |
| DR/BCP (GTM-18) | `platform.dr` | GREEN local; restore/RPO observados en staging; RTO histórico **no califica** | backup versionado; restore aislado `registry-5`; el `DR_SIMULATION` histórico pasó, pero midió RTO antes de `verifyDrReplay` | desplegar cronómetro corregido y repetir staging; ensayar cutover/rollback; QA humana y firmas A/V independientes. No publicar RPO/RTO como garantía ni afirmar failover productivo hasta el cierre | s48 | Staff SRE + Staff Principal |
| Agentic insights / briefing (GTM-10) | `analytics.agentic_insights` | GREEN local + canary fast-path parcial; claim congelado | UI y contrato PII-free/idempotente/anti-alucinación | respuesta generativa válida con facts no triviales; SLO general (no solo fast path); Cron 3:30/KV; QA humana y firmas A+V | s49 | Staff PM + Staff Data |
