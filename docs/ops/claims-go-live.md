...
|---|---|---|---|---|---|---|
| Emisión SUNAT en vivo (GTM-07/08) | `fiscal.cpe` | GREEN local y **e-beta** (pipeline SOAP `sendBill`/`sendSummary` + XAdES Worker; complementary RC) | submit HTTP→PSE (código), SOAP e-beta (piloto TENANT_CERT), CDR, drain outbox→R2→SENT, DLQ | QG GTM-08: mismo ticket **ACCEPTED en D1 y visible en SOL prod o OSE/PSE acreditado**. e-beta **no** basta. S11–S15 WAIT (pass CDT / URL PSE / auth e-factura). Firma A+V. `go-live-sunat` permanece `AGENDADO_AL_FINAL` hasta entonces | GTM-08 / FASE 8 | Staff Fiscal + Staff Security |
