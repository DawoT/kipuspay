...
def main() -> int:
    try:
        ...
        print(f"RESULT V-13 RED  falta {LEDGER}")
        return 1
    ...
    for i, (eid, stored, computed, prev) in enumerate(entries):
        if stored is not None and stored != computed:
            problems.append(f"{eid}: entry_hash {stored[:12]} != calculado {computed[:12]}")
        if prev is not None and i > 0:
            prev_stored = entries[i - 1][1]
            if prev_stored is not None and prev != prev_stored:
                problems.append(f"{eid}: prev_hash {prev[:12]} != entry_hash previo {prev_stored[:12]}")

    hashed = sum(1 for _, stored, _, _ in entries if stored)
    if problems:
        print(f"RESULT V-13 RED  cadena rota en {len(problems)} punto(s)")
        for p in problems[:5]:
            print(f"     {p}")
        return 1
    print(f"RESULT V-13 GREEN")
    print(f"     {len(entries)} entradas, {hashed} con hash verificado")
    return 0
