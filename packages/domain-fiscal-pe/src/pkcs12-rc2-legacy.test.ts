...
function makeP12(args: string[], password: string): Uint8Array {
  ...
  try {
    ...
    execFileSync('openssl', ['req', '-new', '-x509', '-key', key, '-out', cert, '-days', '2', '-subj', '/CN=RC2 Legacy Test/O=Test/C=PE'], { stdio: 'pipe' });
    execFileSync('openssl', ['pkcs12', '-export', '-inkey', key, '-in', cert, '-out', p12, '-passout', `pass:${password}`, ...args, '-legacy'], { stdio: 'pipe' });
...
describe('PKCS#12 RC2-40 legacy (SOL) — fallback vendorizado RFC2268', { timeout: 30000 }, () => {
  it('parsea p12 con cert RC2-40 (key 3DES + cert RC2) — caso SOL típico', async () => {
    const p12 = makeP12(['-keypbe', 'PBE-SHA1-3DES', '-certpbe', 'PBE-SHA1-RC2-40'], 'rc2-pass-legacy');
    ...
    const key = await crypto.subtle.importKey('pkcs8', parsed.pkcs8Der.buffer.slice(parsed.pkcs8Der.byteOffset, parsed.pkcs8Der.byteOffset + parsed.pkcs8Der.byteLength) as ArrayBuffer, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['sign']);
    expect(key).toBeDefined();
  });

  it('parsea p12 con ambos bags RC2-40 (key RC2 + cert RC2) — cobertura OID 1.2.840.113549.1.12.1.6', async () => {
    const p12 = makeP12(['-keypbe', 'PBE-SHA1-RC2-40', '-certpbe', 'PBE-SHA1-RC2-40'], 'rc2-both-pass');
