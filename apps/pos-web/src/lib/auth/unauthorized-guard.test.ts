...
describe('guard de sesión expirada (401 → /login, F5)', () => {
  it('redirige a /login cuando el API responde 401 fuera de la allowlist', async () => {
    ...
    const guard = installUnauthorizedGuard({ fetcher: inner, locationAssign: assign, pathname: '/caja', allowlist: ['/api/auth/session'] });
  ...
  it('no redirige si ya estamos en /login (evita loops)', async () => {
    const assign = vi.fn();
    const inner = vi.fn().mockResolvedValue(jsonResponse({ code: 'UNAUTHENTICATED' }, 401));
    const guard = installUnauthorizedGuard({ fetcher: inner, locationAssign: assign, pathname: '/login', allowlist: ['/api/auth/session'] });
  ...
  it('no redirige en el bootstrap de sesión (allowlist)', async () => {
    const assign = vi.fn();
    const inner = vi.fn().mockResolvedValue(jsonResponse({ code: 'UNAUTHENTICATED' }, 401));
    const guard = installUnauthorizedGuard({ fetcher: inner, locationAssign: assign, pathname: '/', allowlist: ['/api/auth/session'] });
