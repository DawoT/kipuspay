...
describe('poll de captura de pago (S22, F2)', () => {
  it('sondea hasta CAPTURED y devuelve el estado final', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({ id: 'cap-1', status: 'PENDING', acquirer: 'culqi', amount_cents: 1000, sale_id: 's-1' }),
      )
      .mockResolvedValueOnce(
        jsonResponse({ id: 'cap-1', status: 'CAPTURED', acquirer: 'culqi', amount_cents: 1000, sale_id: 's-1' }),
      );
  ...
  it('rinde en FAILED sin seguir sondeando', async () => {
    const fetcher = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ id: 'cap-1', status: 'FAILED', acquirer: 'yape', amount_cents: 500, sale_id: 's-1' }));
    const res = await pollCaptureStatus({
      fetcher,
      apiBase: 'https://api.test',
      authorization: 'Bearer x',
  ...
  it('agota intentos y reporta PENDING sin reventar', async () => {
    const fetcher = vi
      .fn()
      .mockResolvedValue(jsonResponse({ id: 'cap-1', status: 'PENDING', acquirer: 'plin', amount_cents: 500, sale_id: 's-1' }));
    const res = await pollCaptureStatus({
      fetcher,
      apiBase: 'https://api.test',
      authorization: 'Bearer x',
