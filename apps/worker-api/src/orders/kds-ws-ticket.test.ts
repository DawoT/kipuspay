...
describe('KDS WS ticket', () => {
  it('flag off → 404; sin KV → 503; mint + consume one-shot', async () => {
    expect((await runMintKdsWsTicketHttp({ FEATURE_ORDERS_KDS: '0' }, 't1', 'b1')).status).toBe(404);
    expect((await runMintKdsWsTicketHttp({ FEATURE_ORDERS_KDS: '1' }, 't1', 'b1')).status).toBe(503);
