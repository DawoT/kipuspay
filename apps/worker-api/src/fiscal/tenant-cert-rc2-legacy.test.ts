...
function makeP12B64(subj: string, pass: string, keyPbe: string, certPbe: string): string {
  ...
  try {
    ...
    execFileSync('openssl', ['req', '-new', '-x509', '-key', key, '-out', cert, '-days', '300', '-subj', subj], { stdio: 'pipe' });
    execFileSync('openssl', ['pkcs12', '-export', '-inkey', key, '-in', cert, '-out', p12, '-passout', `pass:${pass}`, '-keypbe', keyPbe, '-certpbe', certPbe, '-legacy'], { stdio: 'pipe' });
...
describe('tenant-cert upload RC2-40 legacy (OID 1.2.840.113549.1.12.1.6) — fallback vendorizado', { timeout: 30000 }, () => {
  it('upload 200 con p12 sintético RC2-40 (openssl -legacy) — parse OK', async () => {
    const p12B64 = makeP12B64(cdtSubject(RUC_TENANT), 'rc2-pass-ok', 'PBE-SHA1-3DES', 'PBE-SHA1-RC2-40');
    const wrapDek = vi.fn().mockResolvedValue({ wrappedDek: new Uint8Array(48).fill(7), kekVersion: 'v1' });
    const env = { BACKUP_KMS: { wrapDek }, DB: fakeDb() } as unknown as WorkerEnv;
    const res = await runUploadTenantCertHttp(env, 't1', 'owner', { p12B64, password: 'rc2-pass-ok' });
  ...
  it('upload 200 con ambos bags RC2-40 (key RC2 + cert RC2) — OID 1.2.840.113549.1.12.1.6 doble', async () => {
    const p12B64 = makeP12B64(cdtSubject(RUC_TENANT), 'rc2-both-ok', 'PBE-SHA1-RC2-40', 'PBE-SHA1-RC2-40');
    const wrapDek = vi.fn().mockResolvedValue({ wrappedDek: new Uint8Array(48).fill(7), kekVersion: 'v1' });
    const env = { BACKUP_KMS: { wrapDek }, DB: fakeDb() } as unknown as WorkerEnv;
    const res = await runUploadTenantCertHttp(env, 't1', 'owner', { p12B64, password: 'rc2-both-ok' });
    expect(res.status).toBe(200);
    expect(res.body.fingerprintSha256).toMatch(/^[a-f0-9]{64}$/);
  });

  it('400 con password incorrecta — no filtra secreto, no llama KMS', async () => {
    const p12B64 = makeP12B64(cdtSubject(RUC_TENANT), 'correct-pass', 'PBE-SHA1-3DES', 'PBE-SHA1-RC2-40');
    const wrapDek = vi.fn();
    const env = { BACKUP_KMS: { wrapDek }, DB: fakeDb() } as unknown as WorkerEnv;
    const res = await runUploadTenantCertHttp(env, 't1', 'owner', { p12B64, password: 'wrong-pass' });
