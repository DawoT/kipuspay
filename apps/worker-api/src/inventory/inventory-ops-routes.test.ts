...
describe('submit count review', () => {
  ...
  it('US-04 auditoría :247: diff_value_cents exacto con aritmética entera en montos grandes', async () => {
    ...
    // Cruzarevisión US-02×US-03×US-04: tres implementaciones exactas deben
    // coincidir en el mismo centavo (la ruta conserva la variante US-02).
  ...
  it('US-04 auditoría :247: producto fuera del rango exacto → 422 estable, sin centavo inventado', async () => {
    ...
    // Resolución de conflicto US-02×US-04 (prioridad dinero igual): la ruta
    // conserva computeDiffValueCentsExact (US-02, con resto sub-cent
    // auditable); el guard fail-closed de la auditoría US-04 se mantiene con
    // el código estable que esta ruta emite, sin persistir centavo inventado.
    expect(res.status).toBe(422);
    expect(res.body.code).toBe('COUNT_DIFF_VALUE_OVERFLOW');
  ...
  it('US-05: rechaza costo autoridad degenerado sin coalescer a 0 (422 fail-closed)', async () => {
    for (const degenerate of [null, undefined, 12.5, -5, '100', Number.NaN]) {
      ...
      expect(res.status).toBe(422);
      expect(res.body.code).toBe('COUNT_INVALID_UNIT_COST');
    }
  });
});
...
describe('stock loss', () => {
  it('create valida qty/reason fail-closed (400 tipado, sin ?? 0)', async () => {
    ...
    expect(bad.body.code).toBe('INVALID_LOSS_QTY');

    const coerced = await runCreateStockLossHttp(mockDbEnv(), 't1', 'u1', {
      branchId: 'b1',
      productId: 'p1',
    ...
    expect(coerced.body.code).toBe('INVALID_LOSS_QTY');
...
describe('US-02: diff_value_cents con aritmética entera exacta y guard 2^53', () => {
  ...
  it('route: countedQty tipo string → 400 estable COUNT_INVALID_QUANTITY (antes de D1)', async () => {
    const sqls: string[] = [];
    const res = await runSubmitCountReviewHttp(mockDbEnv({ sqls }), 't1', 'owner', {
      countId: 'c1',
      lines: [{ productId: 'p1', countedQty: '8' as unknown as number }],
    });
    expect(res).toMatchObject({ status: 400, body: { code: 'COUNT_INVALID_QUANTITY' } });
    expect(sqls).toHaveLength(0);
  });

  it('route: countedQtyMicrounits flotante → 400 estable COUNT_INVALID_QUANTITY (antes de D1)', async () => {
    const sqls: string[] = [];
    const res = await runSubmitCountReviewHttp(mockDbEnv({ sqls }), 't1', 'owner', {
      countId: 'c1',
      lines: [{ productId: 'p1', countedQtyMicrounits: 1.5 }],
    });
    expect(res).toMatchObject({ status: 400, body: { code: 'COUNT_INVALID_QUANTITY' } });
    expect(sqls).toHaveLength(0);
  });
